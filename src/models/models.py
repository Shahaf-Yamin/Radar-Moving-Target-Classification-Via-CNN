...
def adjust_input_size(config):
    ...
    if config.use_color_map_representation is True:
        config.__setattr__("model_input_dim", [126, 32, 3])
    return config
...
def build_model(config):
    ...
    elif config.model_name == "cnn-baseline-8layers":
        model = {'train': BaselineCNNModel8layers(config)}
...
def BaselineCNNModel8layers(config):
    def skip_connection_layer(x_orig, x_transformed, filt):
        if x_orig.shape.as_list()[-1] != x_transformed.shape.as_list()[-1]:
            r = x_transformed.shape.as_list()[-1] / x_orig.shape.as_list()[-1]
            assert not (bool(r % 1))
            # apply convolution as transformation
            x_orig = Conv2D(x_transformed.shape.as_list()[-1], kernel_size=filt, padding='same')(x_orig)
        value = Add()([x_orig, x_transformed])
        return value
    def conv2d_layer(x, size, filt):
        if lamda > 0.0:
            x = Conv2D(size, kernel_size=filt, activation='relu', bias_regularizer=keras.regularizers.l2(lamda),
                         kernel_regularizer=keras.regularizers.l2(lamda), padding='same')(x)
        else:
            x = Conv2D(size, kernel_size=filt, activation='relu', padding='same')(x)
        return x
    def dense_layer(x, size):
        if lamda > 0.0:
            x = Dense(dense_list[i], kernel_regularizer=keras.regularizers.l2(lamda),
                  bias_regularizer=keras.regularizers.l2(lamda), activation='relu')(x)
        else:
            x = Dense(dense_list[i], activation='relu')(x)
    ...
    ##########
    # "hidden_size": [16, 16, 32, 32, 64, 64, 128, 128],
    #########
    input_layer = Input(shape=input_shape)
    x = input_layer

    # Layer 1
    x = conv2d_layer(x, 16, (3,3))
    # Layer 2
    x = conv2d_layer(x, 16, (3, 3))
    # Layer 3
    x = conv2d_layer(x, 32, (3, 3))
    x_orig = x
    # Layer 4
    x = conv2d_layer(x, 32, (3, 3))
    x = skip_connection_layer(x_orig, x, (3,3))
    x_orig = x
    # Layer 5
    ...
    x = MaxPooling2D(pool_size=(2, 2))(x)
    # Layer 7
    x = conv2d_layer(x, 128, (3, 3))
    # Layer 8
    x = conv2d_layer(x, 128, (3, 3))
    x = MaxPooling2D(pool_size=(2, 2))(x)
    # Flatten
    x = Flatten()(x)
    if config.dropout_after_all_conv2d != 0:
        x = Dropout(rate=config.dropout_after_all_conv2d)(x)
    # Dense
    for i in range(len(dense_list)):
        x = dense_layer(x, dense_list[i])
    if p_dropout > 0.0:
        x = Dropout(rate=p_dropout)(x)
    # if lamda > 0.0:
    #     x = Dense(1, activation='sigmoid', kernel_regularizer=keras.regularizers.l2(lamda),
    #               bias_regularizer=keras.regularizers.l2(lamda))(x)
    # else:
    #     x = Dense(1, activation='sigmoid')(x)
    x = Dense(1, activation='sigmoid')(x)
...
def BaselineCNNModel(config):
    def conv2d_first_layer(size, filt, pool_shape):
        if lamda > 0.0:
            model.add(Conv2D(size, kernel_size=filt, activation='relu', bias_regularizer=keras.regularizers.l2(lamda),
                         kernel_regularizer=keras.regularizers.l2(lamda), padding='same',input_shape=input_shape))
        else:
            model.add(Conv2D(size, kernel_size=filt, activation='relu', padding='same', input_shape=input_shape))
        if config.use_batch_norm is True:
            model.add(BatchNormalization())
        model.add(MaxPooling2D(pool_size=pool_shape))

    def conv2d_layer(size, filt, pool_shape):
        if lamda > 0.0:
            model.add(Conv2D(size, kernel_size=filt, activation='relu', bias_regularizer=keras.regularizers.l2(lamda),
                         kernel_regularizer=keras.regularizers.l2(lamda), padding='same'))
        else:
            model.add(Conv2D(size, kernel_size=filt, activation='relu', padding='same'))
        if config.use_batch_norm is True:
            model.add(BatchNormalization())
        model.add(MaxPooling2D(pool_size=pool_shape))
    ...
    model = Sequential()
    conv2d_first_layer(size_list[0], filt_list[0], pool_list[0])

    for i in range(1,len(size_list)):
        conv2d_layer(size_list[i], filt_list[i], pool_list[i])
    ...
    for i in range(len(dense_list)):
        if lamda > 0.0:
            model.add(Dense(dense_list[0], kernel_regularizer=keras.regularizers.l2(lamda), bias_regularizer=keras.regularizers.l2(lamda), activation='relu'))
        else:
    ...
    # if lamda > 0.0:
    #     model.add(Dense(1, kernel_regularizer=keras.regularizers.l2(lamda), bias_regularizer=keras.regularizers.l2(lamda), activation='sigmoid'))
    # else:
...
def CNNResBlockModel(config):
    ...
    # Dense
    for i in range(len(dense_list)):
        x = Dense(dense_list[i], kernel_regularizer=regularization(lamda), bias_regularizer=regularization(lamda),
                  kernel_initializer=kernel_initalizer)(x)
        x = activation(activation_name, x)
        if config.use_batch_norm is True:
            x = BatchNormalization()(x)
    ...
    # else:
    #     x = Dense(1, activation='sigmoid', kernel_regularizer=regularization(lamda),
    #               bias_regularizer=regularization(lamda))(x)
    x = Dense(1, activation='sigmoid')(x)
    output_layer = x
    model = Model(input_layer, output_layer)
    if config.learn_background:
