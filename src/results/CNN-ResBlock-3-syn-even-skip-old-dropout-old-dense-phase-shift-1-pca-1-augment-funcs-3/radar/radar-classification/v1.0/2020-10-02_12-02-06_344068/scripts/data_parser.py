...
class DataSetParser(object):
    ...
    def fft(self, iq, window_type='hann',axis=0, window=None):
        ...
        assert window_type == 'hann' or window_type == 'blackman'
        temp_iq = np.copy(iq)
        ...
        smooth_iq = window_option['blackman'](temp_iq, window)
        iq_blackman = np.log(np.abs(np.fft.fft(smooth_iq, axis=axis)))
        plt.figure()
        plt.imshow(self.normalize(iq_blackman))
        plt.title('Blackman')
        plt.savefig('Blackman_Frequency_example')

        smooth_iq = window_option['blackman_nutall'](temp_iq, window)
        iq_nutall = np.log(np.abs(np.fft.fft(smooth_iq, axis=axis)))
        plt.figure()
        ...
        print('Blackman crosscorrelation distance {}'.format(np.linalg.norm(np.correlate(iq_blackman,iq))))

        print('Nutall covriance distance {}'.format(np.linalg.norm(np.cov(iq_nutall-iq))/np.linalg.norm(np.cov(iq))))
        print('Nutall crosscorrelation distance {}'.format(np.linalg.norm(np.correlate(iq_nutall,iq))))
