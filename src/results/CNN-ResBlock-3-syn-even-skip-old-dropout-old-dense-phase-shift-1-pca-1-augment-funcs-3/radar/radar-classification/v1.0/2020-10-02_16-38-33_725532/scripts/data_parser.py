...
class DataSetParser(object):
    ...
    def analyse_PCA_hyperparameters(self,iq_list):
        ...
        for iq,segment_index in zip(iq_list,range(len(iq_list))):
            ...
            for variance,var_ind in zip(var_list,range(var_list.shape[0])):
                ...
                cross_correlation_array[segment_index][var_ind] = np.linalg.norm(scipy.signal.correlate2d(standardized_iq_mat,X_augment[0])) / np.linalg.norm(scipy.signal.correlate2d(standardized_iq_mat,standardized_iq_mat))
        plt.figure()
    ...
    def dump_PCA_data(self, pickle_name, csv_name, validation_indices):
        ...
        with open('{}'.format(pickle_name), 'rb') as pickle_file:
            raw_data = pickle.load(pickle_file)
            # self.analyse_PCA_hyperparameters(iq_list=raw_data["iq_sweep_burst"][0:50])
            # self.analyse_time_shift_hyperparameters(iq_list=raw_data["iq_sweep_burst"][0:100])
