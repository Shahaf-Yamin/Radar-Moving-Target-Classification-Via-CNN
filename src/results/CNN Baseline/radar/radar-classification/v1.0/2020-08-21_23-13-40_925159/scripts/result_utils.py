...
def make_tsne(data, model, labels, path, exp_type, test=None):
  ...
  preds = make_predication(tsne_data)
...
def analyse_model_performance(model,data, path, history):
  ...
  # train_length = sum(1 for elem in train_dataset)
  # train_data = np.empty(train_length)
  # for i, elem in enumerate(train_dataset): train_data[i] = elem
  #
  # val_length = sum(1 for elem in val_dataset)
  # val_data = np.empty(val_length)
  # for i, elem in enumerate(val_dataset): val_data[i] = elem
  ...
  for index, batch in enumerate(train_dataset):
    predication_train.append(model.predict(batch[0]))
    train_y.append(batch[1])

  for index, batch in enumerate(val_dataset):
    predication_val.append(model.predict(batch[0]))
  ...
  make_tsne(predication_train, model, train_y, path,exp_type='train', test=None)
  make_tsne(predication_val, model, val_y, path, exp_type='validation', test=None)
