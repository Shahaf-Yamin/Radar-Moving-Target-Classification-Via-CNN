...
def expand_data_by_interpolation(data_parser,train_data, validation_segments_id_list):

    interpolated_data = data_parser.get_interpolation_data(validation_segments_id_list=validation_segments_id_list)

    # Validate no bias othe interpolated dataf
...
def load_data(config):
    ...
    if config.with_interpolation is True:
        train_data = expand_data_by_interpolation(data_parser=data_parser, train_data=train_data, validation_segments_id_list=validation_data[1]['segment_id'])
