...
# from data.data_loader import generate_test_set
...
class DataSetParser(object):
    def __init__(self, stable_mode, config, read_test_only=False):
        ...
        # self.validation_data, self.train_data = generate_test_set(self.train_data, testset_size)


        if config.with_interpolation is True:
            self.interpolated_data = self.dump_interpolated_data(self.train_data)
    ...
    def preprocess_data(self,raw_iq_matrices):
        if self.config.use_mti_improvement is True:
            iq = np.zeros((raw_iq_matrices.shape[0], raw_iq_matrices.shape[1] - 3, raw_iq_matrices.shape[2]))
            for segment in range(raw_iq_matrices.shape[0]):
                iq[segment] = self.fft(self.mti_improvement(raw_iq_matrices[segment]))
    ...
    def merge_train_pkl_data(self,pkl_raw_data):
        return np.vstack((pkl_raw_data['iq_sweep_burst'], pkl_raw_data['doppler_burst']))
    ...
    def dump_data(self, pickle_name, csv_name):
        ...
        with open('{}'.format(pickle_name), 'rb') as pickle_file:
            ...
            iq = self.merge_train_pkl_data(pkl_raw_data=raw_data)
