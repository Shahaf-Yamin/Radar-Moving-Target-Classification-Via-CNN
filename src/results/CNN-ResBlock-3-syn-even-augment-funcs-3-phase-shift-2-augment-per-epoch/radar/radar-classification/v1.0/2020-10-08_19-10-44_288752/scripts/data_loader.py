...
def load_data(config):
    ...
    NUMBER_OF_SEGEMENTS_AFTER_AUGMENTION = NUMBER_OF_SEGEMENTS_BEFORE_AUGMENTION * (
        len(config.augment_funcs) + config.augment_expansion_number if 'timestep_shift' in config.augment_funcs else 0 + config.augment_expansion_number if 'normal' in config.augment_funcs else 0 + 1)
...
def transform_per_epoch(data, config):
    ...
    def augment_timestep_shift(iq_mat, label):
        shift = tf.cast(tf.sign(tf.random.normal(shape=np.array([1]), mean=0, stddev=1, dtype=tf.float32)), dtype=tf.int32) * tf.constant(
            config.timestep_shift_delta, dtype=tf.int32)
