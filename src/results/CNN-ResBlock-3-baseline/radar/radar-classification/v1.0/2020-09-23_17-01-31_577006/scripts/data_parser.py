...
class DataSetParser(object):
    ...
    def plot_histogram(self):
        ...
        fig.suptitle('Averaged Freq Response Histogram')
        axes[0,0].plot(np.linspace(0,126,126),mean_histogram)
        axes[0,0].set(xlabel='Frequency Bin', ylabel='Count')
        ...
        for segment in range(len(self.train_data[0])):
            for time_step_index in range(self.train_data[0][0].shape[1]):
                ...
                if self.train_data[1]['target_type'] == 'animal':
        ...
        axes[0,1].plot(np.linspace(0, 126, 126),mean_histogram_human)
        axes[0,1].set(xlabel='Frequency Bin', ylabel='Count')
        axes[0,1].set_title('Human')
        ...
        # fig.suptitle('Animal')
        axes[1,0].plot(np.linspace(0,126, 126),mean_histogram_animal)
        axes[1,0].set(xlabel='Frequency Bin', ylabel='Count')
        axes[1,0].set_title('Animal')

        # ax2.imshow(self.train_data[0][0])
        # ax2.set(xlabel='Slow Axis', ylabel='Fast Axis')
        # ax2.set_title('Freq Response Spectrogram')

        fig.savefig('Averaged Histograms')
        ...
        if self.config.use_mti_improvement is True:
            ...
            for segment, segment_id in zip(range(0, iq.shape[0], expansion_size), train_segments_id):
                for pca_index, current_iq_mat in zip(range(expansion_size), self.PCA_time_augmentation(raw_iq_matrices[segment_id])):
                    ...
                    if self.config.with_max_value_on_doppler is True:
                        iq[segment + pca_index] = self.max_value_on_doppler(iq=iq[segment + pca_index], doppler_burst=doppler_burst[segment])
                    else:
                        iq[segment + pca_index] = np.fft.fftshift(iq[segment + pca_index])
                    iq[segment + pca_index] = self.normalize(iq=iq[segment + pca_index])
