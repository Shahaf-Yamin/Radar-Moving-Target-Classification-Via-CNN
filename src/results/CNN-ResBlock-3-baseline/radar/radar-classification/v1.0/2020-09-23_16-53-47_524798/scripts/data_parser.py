    ...
    def plot_histogram(self):
        ...
        for segment in range(len(self.train_data[0])):
            for time_step_index in range(self.train_data[0][0].shape[1]):
                ...
                if self.train_data[1]['se']:
                    mean_histogram[indices] += 1

        mean_histogram /= len(self.train_data[0])

        for time_step_index in range(self.train_data[0][6000].shape[1]):
            indices = (-self.train_data[0][6000][:, time_step_index]).argsort()[:MAX_VALUES_THRESHOLD]
            mean_histogram[indices] += 1

        fig, (ax1, ax2) = plt.subplots(1, 2)
        fig.suptitle('Human')
        ax1.plot(np.linspace(0, 126, 126),mean_histogram)
        ax1.set(xlabel='Frequency Bin', ylabel='Count')
