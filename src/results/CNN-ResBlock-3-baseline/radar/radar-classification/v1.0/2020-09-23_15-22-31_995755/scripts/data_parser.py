...
class DataSetParser(object):
    ...
    def KLT_coeffients(self,a, vec):
        b = np.reshape(a,(a.shape[0],1))
        klt = np.dot(np.transpose(np.conjugate(b)), vec)
        return klt
    ...
    def KLT_aug(self,x, N_req, variance):
        ...
        standardized_iq_mat = x - np.mean(x, axis=0)
        ...
        KL_mat[:, 5:NUMBER_OF_KL_COEEFIECENT] = KL_mat[:, :NUMBER_OF_KL_COEEFIECENT] + VARIANCE_SCALING * (np.multiply(KL_mat[:, :NUMBER_OF_KL_COEEFIECENT], np.random.normal(loc=0, scale=np.sqrt(2) / 2, size=(32, NUMBER_OF_KL_COEEFIECENT)).view(np.complex128)))
        ...
        #
        #     x_aug = []
        #     eigen_values, vec = np.linalg.eigh(cov)
        #
        #     for i in range(32):
        #         klt = self.KLT_coeffients(a=x[:, i], vec=vec)
        #         z = variance * np.random.normal(loc=0, scale=np.sqrt(2) / 2, size=(128, 2)).view(np.complex128)
        #         x_aug.append(np.transpose(np.dot(klt + np.transpose(z),np.transpose(np.conjugate(vec)))))
        #
