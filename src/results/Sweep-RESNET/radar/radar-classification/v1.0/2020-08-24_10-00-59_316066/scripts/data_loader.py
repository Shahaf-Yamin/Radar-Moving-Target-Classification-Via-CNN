...
def augment(data, config):
    ...
    for augment_index in range(config.augment_expansion_number):
        ...
        if 'freq_shift' in augment_funcs:
            data['train'] = data['train'].concatenate(org_train_data.map(lambda iq_mat, label: (
                tf.keras.preprocessing.image.random_shift(iq_mat,wrg=0.01,hrg=0,row_axis=0,col_axis=1,channel_axis=2), label)))
