...
from tensorflow.keras.layers import Layer, Conv1D, Dense, BatchNormalization, LayerNormalization, MaxPooling1D, Flatten, \
    Conv2D, MaxPooling2D
...
def tcn_model(config,
    ...
    if config.tcn_use_tcn_decoder is True:
        x = TCN(nb_filters, kernel_size, nb_stacks, dilations, padding,
                use_skip_connections, dropout_rate, True,
                activation, kernel_initializer, use_batch_norm, use_layer_norm, l2_reg,
                name='tcn_1')(input_layer)
        # x = TCN(2 * nb_filters, kernel_size, nb_stacks, dilations, padding,
        #         use_skip_connections, dropout_rate, True,
        #         activation, kernel_initializer, use_batch_norm, use_layer_norm, l2_reg,
        #         name='tcn_2')(x)
        # x = TCN(4 * nb_filters, kernel_size, nb_stacks, dilations, padding,
        #         use_skip_connections, dropout_rate, True,
        #         activation, kernel_initializer, use_batch_norm, use_layer_norm, l2_reg,
        #         name='tcn_3')(x)
        x = TCN(nb_filters, kernel_size, nb_stacks, dilations, padding,
                use_skip_connections, dropout_rate, False,
                activation, kernel_initializer, use_batch_norm, use_layer_norm, l2_reg,
                name='tcn_decoder')(x)
    else:
        x = TCN(nb_filters, kernel_size, nb_stacks, dilations, padding,
    ...
    if config.tcn_use_cnn_decoder is True:
        if config.tcn_return_sequences is False:
            raise Exception('CNN decoder for TCN should be used only in return sequence meode')
        x = tf.expand_dims(x, -1)
        input_shape = x.shape.as_list()
        x = Conv2D(filters=nb_filters // 2, kernel_size=config.tcn_kernel_size_cnn, padding='same',
                   kernel_initializer=kernel_initializer, input_shape=input_shape)(x)
        x = MaxPooling2D(pool_size=2)(x)
        x = Conv2D(filters=nb_filters // 4, kernel_size=config.tcn_kernel_size_cnn, padding='same',
                   kernel_initializer=kernel_initializer)(x)
        x = MaxPooling2D(pool_size=2)(x)
        x = Flatten()(x)
        x = Dense(nb_filters // 2, activation='relu')(x)
        x = Dense(nb_filters // 4, activation='relu')(x)
    ...
    if not regression:
        # classification
        x = Dense(1)(x)  # changed to 1, binary classification
        ...
        # model.compile(get_opt(), loss=BinaryCrossentropy(), metrics=[accuracy])
    else:
        # regression
...
def get_tcn_model(config):
    ...
    if config.tcn_flattend is True:
        num_feat = 1
        max_len = config.model_input_dim[0] * config.model_input_dim[1]
    elif config.tcn_use_variable_length:
        num_feat = config.model_input_dim[1]
        max_len = None
    else:
        num_feat = config.model_input_dim[1]
        max_len = config.model_input_dim[0]
    nb_filters = config.tcn_nb_filters
    kernel_size = config.tcn_kernel_size
    dilations = [2 ** i for i in range(config.tcn_dilations_len)]
    nb_stacks = config.tcn_nb_stacks
    use_skip_connections = config.tcn_use_skip_connection
    return_sequences = config.tcn_return_sequences
    use_layer_norm = config.tcn_use_layer_norm
    use_batch_norm = config.tcn_use_batch_norm
    dropout_rate = config.tcn_dropout_rate
    l2_reg = config.tcn_l2_reg
    return tcn_model(num_feat=num_feat, num_classes=num_classes, nb_filters=nb_filters, kernel_size=kernel_size,
                     dilations=dilations, nb_stacks=nb_stacks, max_len=max_len,
                     use_skip_connections=use_skip_connections, config=config, dropout_rate=dropout_rate,
                     return_sequences=return_sequences, use_layer_norm=use_layer_norm, use_batch_norm=use_batch_norm,
                     l2_reg=l2_reg)
