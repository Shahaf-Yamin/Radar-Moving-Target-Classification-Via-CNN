...
def generate_test_set(train_data, TRACK_ID_TEST_SET_SIZE):
    ...
    sorted_segments_id = np.sort(labels_train['segment_id'])

    for segment_num,new_index in zip(sorted_segments_id,sorted_segments_id.shape[0]):
        labels_train['segment_id'][labels_train['segment_id'].index(segment_num)] = new_index
