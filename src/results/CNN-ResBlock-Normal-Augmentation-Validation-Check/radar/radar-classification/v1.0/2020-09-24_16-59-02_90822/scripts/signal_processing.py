...
def PCA_expansion(iq_mat,config):
    ...
    KL_mat[:, 5:NUMBER_OF_KL_COEEFIECENT] = KL_mat[:, :NUMBER_OF_KL_COEEFIECENT] + VARIANCE_SCALING * (np.multiply(KL_mat[:, :NUMBER_OF_KL_COEEFIECENT],np.random.randn(32, NUMBER_OF_KL_COEEFIECENT)))
...
def Sample_rectangle_from_spectrogram(iq_mat,config):
    ...
    new_iq_list = []
    doppler_burst = np.argmax(iq_mat, axis=0)# Extract the doppler burst from the matrix
    for row_ind in range(MAX_NUM_OF_BINS-NUM_OF_ROWS):
        for col_ind in range(MAX_NUM_OF_TIMESTEPS-NUM_OF_COLS):
            ...
            if discard_rect_flag:
                continue
            new_iq_list.append(iq_mat[row_ind:row_ind+NUM_OF_ROWS,col_ind:col_ind+NUM_OF_COLS])

    figure, axes = plt.subplots(nrows=2, ncols=2)
    axes[0, 0].imshow(iq_mat)
    axes[0, 0].set_title('Freq Response Org')
    axes[0, 1].imshow(new_iq_list[0])
    axes[0, 1].set_title('Freq Response Extracted 1 ')
    axes[1, 0].imshow(new_iq_list[1])
    axes[1, 0].set_title('Freq Response Extracted 2 ')
    axes[1, 1].imshow(new_iq_list[2])
    axes[1, 1].set_title('Freq Response Extracted 3 ')
