...
def expand_data_by_synthetic_set(data_parser, train_data,config):
...
def load_data(config):
    ...
    if config.load_pkl_data:
        ...
        if config.load_synthetic is True:
            train_data = expand_data_by_synthetic_set(data_parser=data_parser, train_data=train_data,config=config)
...
def read_data(config):
    ...
    if config.snr_type == 'all':
        train_data = data_parser.get_dataset_by_snr(dataset_type='train', snr_type='all')
