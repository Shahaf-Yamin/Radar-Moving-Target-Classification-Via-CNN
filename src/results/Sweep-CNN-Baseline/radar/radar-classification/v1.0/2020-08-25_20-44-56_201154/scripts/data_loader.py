...
def augment(data, config):
    ...
    def augment_freq_shift(iq_mat,label):
        if config.shift_freq_dc_width == 0:
            return np.roll(iq_mat, config.freq_shift_delta,axis=0), label
        else:
            shiftwidth = config.shift_freq_dc_width
            iq_mat[shiftwidth:-shiftwidth] = np.roll(iq_mat[shiftwidth:-shiftwidth],config.freq_shift_delta, axis=0)
            return iq_mat, label

    def augment_timestep_shift(iq_mat,label):
        return np.roll(iq_mat, config.timestep_shift_delta,axis=1), label
