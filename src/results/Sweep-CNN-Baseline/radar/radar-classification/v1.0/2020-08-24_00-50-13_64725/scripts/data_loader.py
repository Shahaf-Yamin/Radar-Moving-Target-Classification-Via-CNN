...
def augment(data, config):
    ...
    for augment_index in range(config.augment_expansion_number):
        ...
        if 'freq_shift' in augment_funcs:
            data['train'] = data['train'].concatenate(org_train_data.map(lambda iq_mat, label: (augment_freq_shift(iq_mat.numpy(), label))))
        ...
        if 'timestep_shift' in augment_funcs:
            data['train'] = data['train'].concatenate(org_train_data.map(lambda iq_mat, label: (augment_timestep_shift(iq_mat.numpy(), label))))
