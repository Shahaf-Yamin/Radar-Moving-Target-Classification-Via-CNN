...
def augment(data, config):
    ...
    def augment_gaussian_filt_2d(iq_mat, label):
        iq_mat = tf.convert_to_tensor(gaussian_filter(tf.make_ndarray(iq_mat), sigma=1))
        return iq_mat, label

    def augment_freq_shift(iq_mat,label):
        image_augmentor_ty.fit(tf.make_ndarray(iq_mat))
        return tf.convert_to_tensor(image_augmentor_ty.flow(tf.make_ndarray(iq_mat))), label

    def augment_horiz_flip(iq_mat,label):
        image_augmentor_flip_x.fit(tf.make_ndarray(iq_mat))
        return tf.convert_to_tensor(image_augmentor_flip_x.flow(tf.make_ndarray(iq_mat))), label

    def augment_timestep_shift(iq_mat,label):
        image_augmentor_tstep_shift.fit(tf.make_ndarray(iq_mat))
        return tf.convert_to_tensor(image_augmentor_tstep_shift.flow(tf.make_ndarray(iq_mat))), label

    image_augmentor_ty = tf.keras.preprocessing.image.ImageDataGenerator(height_shift_range=config.freq_shift_delta)
    image_augmentor_flip_x = tf.keras.preprocessing.image.ImageDataGenerator(horizontal_flip=True)
    image_augmentor_tstep_shift = tf.keras.preprocessing.image.ImageDataGenerator(width_shift_range=config.timestep_shift_delta)
    ...
    for augment_index in range(config.augment_expansion_number):
        ...
        if 'freq_shift' in augment_funcs:
            data['train'] = data['train'].concatenate(org_train_data.map(lambda iq_mat, label: (augment_freq_shift(iq_mat, label))))
        if 'horiz_flip' in augment_funcs:
            data['train'] = data['train'].concatenate(org_train_data.map(lambda iq_mat, label: (augment_horiz_flip(iq_mat, label))))
        if 'timestep_shift' in augment_funcs:
            data['train'] = data['train'].concatenate(org_train_data.map(lambda iq_mat, label: (augment_timestep_shift(iq_mat, label))))
