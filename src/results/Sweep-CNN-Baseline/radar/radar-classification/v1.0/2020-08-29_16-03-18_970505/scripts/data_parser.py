...
class DataSetParser(object):
    ...
    def mti_improvement(self, iq_mat):
        improved_iq = np.zeros((iq_mat.shape[0],iq_mat.shape[1] -1 ,iq_mat.shape[2]))
        improved_iq[0,:,:] = iq_mat[0,:,:]
        for fast_time_sample_index in range(1,iq_mat.shape[0] - 1):
            improved_iq[fast_time_sample_index,:,:] = iq_mat[fast_time_sample_index,:,:] - iq_mat[fast_time_sample_index - 1,:,:]
        return improved_iq
