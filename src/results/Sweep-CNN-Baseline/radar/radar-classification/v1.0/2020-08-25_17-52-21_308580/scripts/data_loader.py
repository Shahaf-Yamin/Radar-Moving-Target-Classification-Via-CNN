...
def augment(data, config):
    ...
    for augment_index in range(config.augment_expansion_number):
        if 'normal' in augment_funcs:
            data = np.concatenate((data, np.array([augment_normal(sample[0],sample[1]) for sample in data])),axis=0)
        if 'guassian_filt' in augment_funcs:
            data = np.concatenate((data, np.array([augment_gaussian_filt_2d(sample[0],sample[1]) for sample in data])),axis=0)
        if 'flip_image' in augment_funcs:
            new_arr = np.expand_dims(np.stack(np.swapaxes(data,0,1)[0]),axis=3)
            image_augmentor_flip_y.fit(new_arr)
            z = image_augmentor_ty.flow(new_arr)
            data = np.concatenate((data, np.array([augment_vertical_flip(sample[0],sample[1]) for sample in data])),axis=0)
        if 'freq_shift' in augment_funcs:
            image_augmentor_ty.fit(np.expand_dims(np.stack(np.swapaxes(data,0,1)[0]),axis=3))
            data = np.concatenate((data, np.array([augment_freq_shift(sample[0],sample[1]) for sample in data])),axis=0)
        if 'horiz_flip' in augment_funcs:
            image_augmentor_flip_x.fit(np.expand_dims(np.stack(np.swapaxes(data,0,1)[0]),axis=3))
            data = np.concatenate((data, np.array([augment_horiz_flip(sample[0],sample[1]) for sample in data])),axis=0)
        if 'timestep_shift' in augment_funcs:
            image_augmentor_tx.fit(np.expand_dims(np.stack(np.swapaxes(data,0,1)[0]),axis=3))
            data = np.concatenate((data, np.array([augment_timestep_shift(sample[0],sample[1]) for sample in data])),axis=0)
    return data
