...
def augment(data, config):
    ...
    def augment_freq_shift(iq_mat,label):
        ...
        return tf.convert_to_tensor(image_augmentor_ty.flow(iq_mat)), label

    def augment_horiz_flip(iq_mat,label):
        image_augmentor_flip_x.fit(iq_mat)
        return tf.convert_to_tensor(image_augmentor_flip_x.flow(iq_mat)), label

    def augment_timestep_shift(iq_mat,label):
        image_augmentor_tstep_shift.fit(iq_mat)
        return tf.convert_to_tensor(image_augmentor_tstep_shift.flow(iq_mat)), label
    ...
    for augment_index in range(config.augment_expansion_number):
        ...
        if 'guassian_filt' in augment_funcs:
            data['train'] = data['train'].concatenate(org_train_data.map(lambda iq_mat, label: (augment_gaussian_filt_2d(np.array(iq_mat), label))))
        ...
        if 'freq_shift' in augment_funcs:
            data['train'] = data['train'].concatenate(org_train_data.map(lambda iq_mat, label: (augment_freq_shift(np.array(iq_mat), label))))
        if 'horiz_flip' in augment_funcs:
            data['train'] = data['train'].concatenate(org_train_data.map(lambda iq_mat, label: (tf.image.flip_left_right(iq_mat), label)))
        if 'timestep_shift' in augment_funcs:
            data['train'] = data['train'].concatenate(org_train_data.map(lambda iq_mat, label: (augment_timestep_shift(np.array(iq_mat), label))))
