...
def load_data(config):
    ...
    train_data = convert_metadata_to_numpy(train_data)
...
def augment(data, config):
    ...
    def augment_freq_shift(iq_mat,label):
        image_augmentor_ty.fit(iq_mat)
        return image_augmentor_ty.flow(iq_mat), label

    def augment_timestep_shift(iq_mat,label):
        image_augmentor_tx.fit(iq_mat)
        return image_augmentor_tx.flow(iq_mat), label

    def augment_vertical_flip(iq_mat,label):
        image_augmentor_flip_y.fit(iq_mat)
        return image_augmentor_tx.flow(iq_mat), label

    def augment_horiz_flip(iq_mat,label):
        image_augmentor_flip_x.fit(iq_mat)
        return image_augmentor_tx.flow(iq_mat), label

    # Set image augmentor objects
    image_augmentor_ty = tf.keras.preprocessing.image.ImageDataGenerator(height_shift_range=config.freq_shift_delta, fill_mode='wrap')
    image_augmentor_flip_x = tf.keras.preprocessing.image.ImageDataGenerator(horizontal_flip=True)
    image_augmentor_tx = tf.keras.preprocessing.image.ImageDataGenerator(width_shift_range=config.timestep_shift_delta, fill_mode='wrap')
    image_augmentor_flip_y = tf.keras.preprocessing.image.ImageDataGenerator(vertical_flip=True)
    ...
    for augment_index in range(config.augment_expansion_number):
        if 'normal' in augment_funcs:
            ...
            data = np.concatenate((data,np.array([augment_normal(iq_mat,labels[label_index]) for iq_mat, label_index in zip(data[0], range(iq_matrices.shape[0]))])),axis=0)
        if 'guassian_filt' in augment_funcs:
            data = np.concatenate((data, np.array([augment_gaussian_filt_2d(iq_mat,labels[label_index]) for iq_mat, label_index in zip(data[0], range(iq_matrices.shape[0]))])),axis=0)
        if 'flip_image' in augment_funcs:
            data = np.concatenate((data, np.array([augment_vertical_flip(iq_mat,labels[label_index]) for iq_mat, label_index in zip(data[0], range(iq_matrices.shape[0]))])),axis=0)
        if 'freq_shift' in augment_funcs:
            data = np.concatenate((data, np.array([augment_freq_shift(iq_mat,labels[label_index]) for iq_mat, label_index in zip(data[0], range(iq_matrices.shape[0]))])),axis=0)
        if 'horiz_flip' in augment_funcs:
            data = np.concatenate((data, np.array([augment_horiz_flip(iq_mat,labels[label_index]) for iq_mat, label_index in zip(data[0], range(iq_matrices.shape[0]))])),axis=0)
        if 'timestep_shift' in augment_funcs:
            data = np.concatenate((data, np.array([augment_timestep_shift(iq_mat,labels[label_index]) for iq_mat, label_index in zip(data[0], range(iq_matrices.shape[0]))])),axis=0)
    return data
