...
class DataSetParser(object):
    ...
    def interpolation(self, iq_time_matrix):
        ...
        fast_axis = np.arange(0, 126)
        mesh_fast_axis, mesh_slow_axis = np.meshgrid(slow_axis, fast_axis)

        '''
        Generating the interpolator object
        '''
        interpolated_iq_time_func_real = interpolate.interp2d(mesh_slow_axis, mesh_fast_axis, np.real(iq_time_matrix), kind='quintic')
        interpolated_iq_time_func_imag = interpolate.interp2d(mesh_slow_axis, mesh_fast_axis, np.imag(iq_time_matrix), kind='quintic')

        '''
        Creating the augmented axis
        '''
        slow_axis_interpolated = np.arange(0, 32, 1. / self.config.interpolation_size)
        fast_axis_interpolated = np.arange(0, 126, 1. / self.config.interpolation_size)

        interpolated_iq_mat_real = interpolated_iq_time_func_real(slow_axis_interpolated, fast_axis_interpolated)
        ...
        for index in range(self.config.interpolation_size):
            ...
            fast_stop_index = self.config.fast_time_interpolation_size * 126
            slow_stop_index = self.config.fast_time_interpolation_size * 32
            current_fast_axis = np.arange(start_index, fast_stop_index, self.config.fast_time_interpolation_size)
            current_slow_axis = np.arange(start_index, slow_stop_index, self.config.fast_time_interpolation_size)
            interpolated_iq_list.append(interpolated_iq_mat_real[current_slow_axis][current_fast_axis]
                                        + 1j * interpolated_iq_mat_imag[current_slow_axis][current_fast_axis])
