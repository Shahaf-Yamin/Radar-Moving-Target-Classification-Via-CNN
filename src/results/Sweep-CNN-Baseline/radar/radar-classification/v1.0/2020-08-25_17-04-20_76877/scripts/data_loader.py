...
def augment(data, config):
    ...
    for augment_index in range(config.augment_expansion_number):
        if 'normal' in augment_funcs:
            data = np.concatenate((data,np.array([augment_normal(iq_mat,label) for iq_mat,label in data])),axis=0)
        if 'guassian_filt' in augment_funcs:
            data = np.concatenate((data, np.array([augment_gaussian_filt_2d(iq_mat, label) for iq_mat, label in data])), axis=0)
        if 'flip_image' in augment_funcs:
            data = np.concatenate((data, np.array([augment_vertical_flip(iq_mat, label) for iq_mat, label in data])), axis=0)
        if 'freq_shift' in augment_funcs:
            data = np.concatenate((data, np.array([augment_freq_shift(iq_mat, label) for iq_mat, label in data])), axis=0)
        if 'horiz_flip' in augment_funcs:
            data = np.concatenate((data, np.array([augment_horiz_flip(iq_mat, label) for iq_mat, label in data])), axis=0)
        if 'timestep_shift' in augment_funcs:
            data = np.concatenate((data, np.array([augment_timestep_shift(iq_mat, label) for iq_mat, label in data])), axis=0)
    return data
