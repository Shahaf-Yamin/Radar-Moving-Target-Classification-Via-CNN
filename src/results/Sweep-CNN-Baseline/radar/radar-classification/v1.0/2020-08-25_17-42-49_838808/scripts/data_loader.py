...
def augment(data, config):
    ...
    for augment_index in range(config.augment_expansion_number):
        ...
        if 'flip_image' in augment_funcs:
            image_augmentor_flip_y.fit(np.expand_dims(np.swapaxes(data,0,1)[0],axis=2))
        ...
        if 'freq_shift' in augment_funcs:
            image_augmentor_ty.fit(np.expand_dims(np.swapaxes(data, 0, 1)[0], axis=2))
        ...
        if 'horiz_flip' in augment_funcs:
            image_augmentor_flip_x.fit(np.expand_dims(np.swapaxes(data, 0, 1)[0], axis=2))
        ...
        if 'timestep_shift' in augment_funcs:
            image_augmentor_tx.fit(np.expand_dims(np.swapaxes(data, 0, 1)[0], axis=2))
