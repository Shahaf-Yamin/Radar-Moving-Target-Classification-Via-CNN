...
def transform(data):
    ...
    data['train'] = data['train'].map(transform_example).take(-1)
    data['train_eval'] = data['train_eval'].map(transform_example).take(-1)
...
def make_iterators(data, config):
    ...
    n_train = 150000
    if config.augment_per_epoch is True:
        train_iter = data['train'].shuffle(n_train + 1000).repeat().batch(config.batch_size, drop_remainder=True).prefetch(n_train + 1000)
    else:
        train_iter = data['train'].shuffle(n_train + 1000).batch(config.batch_size, drop_remainder=True)
