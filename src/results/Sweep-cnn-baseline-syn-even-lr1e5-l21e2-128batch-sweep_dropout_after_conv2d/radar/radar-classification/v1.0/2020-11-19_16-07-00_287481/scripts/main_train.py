...
from models.models import build_or_load_model,adjust_input_size
...
def main():
    ...
    assert not config.merge_train_and_public
    # load the data
    ...
    with strategy.scope():
        ...
        # create trainer
        trainer = build_trainer(model, data, config, exp_name_time)
    ...
    if config.save_model is True:
        ...
        save_model(name='{}'.format(exp_name_time), model=model['train'])
