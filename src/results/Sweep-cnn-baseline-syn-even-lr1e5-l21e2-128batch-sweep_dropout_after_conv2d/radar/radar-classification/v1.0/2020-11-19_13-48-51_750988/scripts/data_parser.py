...
class DataSetParser(object):
    ...
    def preprocess_pkl_data(self, pkl_raw_data):
        ...
        if self.config.with_iq_matrices is True or self.config.with_magnitude_phase is True:
            iq = np.zeros((raw_iq_matrices.shape[0] * self.config.freq_expansion_interpolation, raw_iq_matrices.shape[1] - 2, raw_iq_matrices.shape[2], 2))
        else:
            iq = np.zeros((raw_iq_matrices.shape[0] * self.config.freq_expansion_interpolation, raw_iq_matrices.shape[1] - 2, raw_iq_matrices.shape[2]))
