...
def expand_data_by_pca_time_comperssion(data_parser, train_data, validation_segments_id_list):
    ...
    file_name = data_parser.train_file_to_load
    augmentad_data = data_parser.dump_PCA_data(pickle_name='{}.pkl'.format(file_name),
...
def expand_data_by_freq_rotation(data_parser, train_data, validation_segments_id_list):
    file_name = data_parser.train_file_to_load
    augmentad_data = data_parser.dump_freq_rotation_data(pickle_name='{}.pkl'.format(file_name),
                                                         csv_name='{}.csv'.format(file_name),
                                                         validation_indices=validation_segments_id_list)
    for segment_id in augmentad_data[1]['segment_id']:
        if segment_id[1:] in validation_segments_id_list:
...
def expand_data_by_phase_time_shift(data_parser, train_data, validation_segments_id_list):
    # Validate that the interpolated data set has no bias with the validation set
    file_name = data_parser.train_file_to_load
    augmentad_data = data_parser.dump_time_shift_using_phase_data(pickle_name='{}.pkl'.format(file_name),
                                                                  csv_name='{}.csv'.format(file_name),
                                                                  validation_indices=validation_segments_id_list)
...
def expand_data_by_window_function(data_parser, train_data, validation_segments_id_list, config):
    ...
    for window in config.window_list:
        augmentad_data = data_parser.dump_window_augmentation(window_type=window, pickle_name='{}.pkl'.format(file_name),
                                                              csv_name='{}.csv'.format(file_name),
                                                              validation_indices=validation_segments_id_list)
...
def expand_data_by_public_test(train_data,data_parser):
    public_test_data, public_train_data = data_parser.split_public_test_valid()
    X = np.concatenate((train_data[0], public_train_data[0]), axis=0)

    for key in train_data[1].keys():
        aux_test_arr = np.array(public_train_data[1][key])
        train_data[1][key].extend(aux_test_arr.tolist())

    train = X, train_data[1]
    validation = public_test_data
...
def load_data(config):
    if config.load_pkl_data:
        ...
        if config.with_public_test_set_training is True:
            train_data, validation_data = expand_data_by_public_test(train_data,data_parser)
        ...
        if config.use_color_map_representation:
            color_map_representation(train_data, validation_data)

        if config.repeat_numpy_data_set > 0:
            assert config.augment_per_epoch != True
            orig_data = train_data
            for i in range(config.repeat_numpy_data_set):
                train_data = np.concatenate((train_data, orig_data), axis=0)
        ...
        if config.save_pkl_data is True:
            ...
            with open('Organized_Data_in_pickle/{}_data_description.txt'.format(config.pickle_file_name), mode='w') as data_file:
                ...
                data_file.write('No. segments in train: {}\n'.format(len(train_data)))
                data_file.write('No. segments in validation: {}\n'.format(len(validation_data)))
                data_file.write('train targets: {} humans, {} animals\n'.format(len(train_data[:, 1][train_data[:, 1] == 1]),
                                                                              len(train_data[:, 1][train_data[:, 1] == 0])))
                data_file.write('validation targets: {} humans, {} animals\n'.format(len(validation_data[:, 1][validation_data[:, 1] == 1]),
...
def augment(data, config):
    def print_augmentations_for_presentation(X0):
        # original
        plt.figure()
        plt.imshow(X0)
        plt.title('Original')
        ...
        plt.savefig('X0.png', transparent=True)

        # vertical flip
        X0_flipped, _ = augment_vertical_flip(X0, 0)
        plt.figure()
        plt.imshow(X0_flipped)
        plt.title('Vertical flip')
        plt.xlabel('slow time')
        plt.ylabel('fast time')
        plt.savefig('X0_flipped.png', transparent=True)

        # horizontal flip
        X0_horiz_flipped, _ = augment_horiz_flip(X0, 0)
        plt.figure()
        plt.imshow(X0_horiz_flipped)
        plt.title('Horizontal flip')
        plt.xlabel('slow time')
        plt.ylabel('fast time')
        plt.savefig('X0_horiz_flipped.png', transparent=True)

        # timestep shift
        X0_timestep, _ = augment_timestep_shift(X0, 0)
        plt.figure()
        plt.imshow(X0_timestep)
        plt.title('Cyclic random time shift')
        plt.xlabel('slow time')
        plt.ylabel('fast time')
        plt.savefig('X0_timestep.png', transparent=True)

        # normal 0.5
        config['augment_normal_std'] = 0.5
        X0_normal_05, _ = augment_normal(X0, 0)
        plt.figure()
        plt.imshow(X0_normal_05)
        plt.title('normal 0.5')
        plt.xlabel('slow time')
        plt.ylabel('fast time')
        plt.savefig('X0_normal_05.png', transparent=True)

        # normal 0.75
        config['augment_normal_std'] = 0.75
        X0_normal_075, _ = augment_normal(X0, 0)
        plt.figure()
        plt.imshow(X0_normal_075)
        plt.title('normal 0.75')
        plt.xlabel('slow time')
        plt.ylabel('fast time')
        plt.savefig('X0_normal_075.png', transparent=True)

        # normal 1.0
        config['augment_normal_std'] = 1.0
        X0_normal_1, _ = augment_normal(X0, 0)
        plt.figure()
        plt.imshow(X0_normal_1)
        plt.title('normal 1.0')
        plt.xlabel('slow time')
        plt.ylabel('fast time')
        plt.savefig('X0_normal_1.png', transparent=True)

    def normalize(iq_mat):
        iq_mat = np.maximum(np.median(iq_mat) - 1, iq_mat)
        iq_mat = (iq_mat - np.mean(iq_mat)) / np.std(iq_mat)
        return iq_mat

    def augment_normal(iq_mat, label):
        iq_mat = iq_mat + np.random.normal(loc=config.augment_normal_mean, scale=config.augment_normal_std,
                                           size=iq_mat.shape)
        return normalize(iq_mat), label
    ...
    for augment_index in range(config.augment_expansion_number):
        ...
        # print_augmentations_for_presentation(orig_data[0][0])
...
def make_iterators(data, config):
    ...
    n_train = sum(1 for _ in data['train'].as_numpy_iterator())
    if config.augment_per_epoch is True:
        train_iter = data['train'].shuffle(n_train + 1000, reshuffle_each_iteration=True).repeat().batch(config.batch_size, drop_remainder=True).prefetch(config.batch_size).take(-1)
    else:
        train_iter = data['train'].shuffle(n_train + 1000, reshuffle_each_iteration=True).batch(config.batch_size, drop_remainder=True).take(-1)
...
def color_map_representation(train_data, validation_data):
    # for i, (img,label) in zip(range(train_data.shape[0]),train_data):
    #     rgb_img = cv2.cvtColor(img, cv2.COLOR_GRAY2RGB)
    #     train_data[i,0] = rgb_img
    return