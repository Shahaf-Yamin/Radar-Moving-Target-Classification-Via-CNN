...
class DataSetParser(object):
    ...
    def preprocess_pkl_data(self, pkl_raw_data):
        ...
        if self.config.with_iq_matrices is True or self.config.with_magnitude_phase is True:
            ...
            if self.config.with_slow_time_interpolation is True:
                interpolated_iq = np.zeros((raw_iq_matrices.shape[0],(raw_iq_matrices.shape[1] - 2) * self.config.freq_expansion_interpolation, self.config.slow_time_interpolation))
        ...
        for segment in range(raw_iq_matrices.shape[0]):
            ...
            if self.config.with_slow_time_interpolation is True:
                interpolated_iq[segment] = resample(self.normalize(iq[segment]), self.config.slow_time_interpolation, t=None, axis=1, window=None)
            else:
                iq[segment] = self.normalize(iq=iq[segment])

        if self.config.with_slow_time_interpolation:
            return interpolated_iq
        else:
            return iq
