...
def sweep_core(config, graph_path, res_dir,best_preformance_dir,current_sweep,param_value):
    ...
    # create trainer
    trainer = build_trainer(model, data, config, os.path.basename(graph_path),sweep_string='{}_{}'.format(current_sweep, param_value))
...
def main_sweep():
    ...
    orig_config = config
    config.__setattr__('save_model_periodically',False)
    config.__setattr__('save_model',False)
    config.__setattr__('model_checkpoint',False)
    ...
    for list_name in sweep_list:
        ...
        print_sweep_by_parameter(hist_dict, param_name=param_name, metric_list=['val_loss', 'loss'],
                                 graph_path=graph_path, title='{} loss'.format(param_name))
        if config.learn_background is False:
