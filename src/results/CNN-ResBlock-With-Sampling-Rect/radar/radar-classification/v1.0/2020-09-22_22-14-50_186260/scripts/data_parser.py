...
def KLT(img):
    ...
    for i in range(img.shape[1]):
        cov_mat += (np.matmul(vec[i] - mean, np.transpose(np.conjugate(vec[i] - mean))) / img.shape[1])
    return mean, cov_mat


def KLT_coeffients(a, covariance):
    eigen_values, vec = np.linalg.eig(covariance)
    klt = np.dot(vec, a)
...
class DataSetParser(object):
    ...
    def preprocess_pkl_data(self, pkl_raw_data):
        ...
        if self.config.use_mti_improvement is True:
            ...
            for segment in range(raw_iq_matrices.shape[0]):
                augment_segment = self.KLT_aug(raw_iq_matrices[segment])
    ...
    def KLT_aug(self, iq):
        imgs = []
        for i in range(self.config.num_of_pca_segments):

            mean, cov = KLT(img=iq)
            eigen_val, eigen_vec, klt = KLT_coeffients(a=iq[:, 0], covariance=cov)

            x_aug = []

            for i in range(32):
                noise = self.config.pca_augmentation_scaling * np.random.normal(loc=0, scale=np.sqrt(2) / 2, size=(iq.shape[0], 2)).view(np.complex128)
                x_aug.append(np.dot(np.conjugate(eigen_vec), klt + noise))

            x_aug = np.array(x_aug)
            x_aug = np.reshape(x_aug, (126, 32))
            x_aug = self.fft(iq=x_aug)
            x_aug = np.fft.fftshift(x_aug)
...
def KLT(img):
    vec = [np.reshape(img[:, i], (img.shape[0], 1)) for i in range(img.shape[1])]
    mean = np.zeros((img.shape[0], 1))

    for i in range(img.shape[1]):
        mean += (vec[i] / img.shape[1])

    cov_mat = np.zeros((img.shape[0], img.shape[0]))
...
def KLT_aug(x, N_req, variance):
    imgs = []
    for i in range(N_req):

        mean, cov = KLT(img=x)
        eigen_val, eigen_vec, klt = KLT_coeffients(a=x[:, 0], covariance=cov)

        x_aug = []

        for i in range(32):
            z = variance * np.random.normal(loc=0, scale=np.sqrt(2) / 2, size=(128, 2)).view(np.complex128)
            x_aug.append(np.dot(np.conjugate(eigen_vec), klt + z))

        x_aug = np.array(x_aug)
        x_aug = np.reshape(x_aug, (126, 32))
        x_aug = fft(iq=x_aug)
        x_aug = normalize(iq=x_aug)
        imgs.append(x_aug)
