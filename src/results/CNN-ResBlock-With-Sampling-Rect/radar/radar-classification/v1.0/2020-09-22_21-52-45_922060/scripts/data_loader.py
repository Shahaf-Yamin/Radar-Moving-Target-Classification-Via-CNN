...
def augment(data, config):
    ...
    def augment_freq_shift(iq_mat,label):
        image_augmentor_ty.fit(iq_mat.numpy())
        return tf.convert_to_tensor(image_augmentor_ty.flow(iq_mat.numpy())), label

    def augment_horiz_flip(iq_mat,label):
        image_augmentor_flip_x.fit(iq_mat.numpy())
        return tf.convert_to_tensor(image_augmentor_flip_x.flow(iq_mat.numpy())), label

    def augment_timestep_shift(iq_mat,label):
        image_augmentor_tstep_shift.fit(iq_mat.numpy())
        return tf.convert_to_tensor(image_augmentor_tstep_shift.flow(iq_mat.numpy())), label
