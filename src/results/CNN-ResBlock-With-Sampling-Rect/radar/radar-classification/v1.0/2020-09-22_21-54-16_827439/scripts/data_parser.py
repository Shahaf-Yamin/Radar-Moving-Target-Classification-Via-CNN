...
def KLT(img):
    ...
    mean = np.zeros((img.shape[0], 1)).astype(np.complex128)

    for i in range(img.shape[1]):
        mean += (vec[i] / img.shape[1].astype(np.complex128))

    cov_mat = np.zeros((img.shape[0], img.shape[0]))

    for i in range(img.shape[1]):
        cov_mat += (np.matmul(vec[i] - mean, np.transpose(vec[i] - mean)) / img.shape[1])
    return mean, cov_mat
