...
def Sample_rectangle_from_spectrogram(iq_mat,config,label):
        ...
        for col_ind in range(MAX_NUM_OF_TIMESTEPS-NUM_OF_COLS):
            for col_sub_ind in range(col_ind, col_ind+NUM_OF_COLS):
                ...
                if discard_rect_flag:
                    continue
            new_iq_list.append(iq_mat[row_ind:NUM_OF_ROWS,col_ind:NUM_OF_COLS])
            new_iq_labels.append(label)

    return new_iq_list, new_iq_labels
