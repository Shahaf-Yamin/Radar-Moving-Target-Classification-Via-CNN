...
def PCA_expansion(iq_mat,config):
    ...
    vec = [np.reshape(iq_mat[:, i], (iq_mat.shape[0], 1)) for i in range(iq_mat.shape[1])]
    mean = np.zeros((iq_mat.shape[0], 1))

    for i in range(iq_mat.shape[1]):
        mean += (vec[i] / iq_mat.shape[1])

    cov_mat = np.zeros((iq_mat.shape[0], iq_mat.shape[0]))

    for index in range(iq_mat.shape[1]):
        cov_mat += np.matmul(vec[index] - mean,np.transpose(vec[index] - mean)) / iq_mat.shape[1]
