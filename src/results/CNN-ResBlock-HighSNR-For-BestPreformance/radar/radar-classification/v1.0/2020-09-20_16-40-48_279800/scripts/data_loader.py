...
def generator(X,y,config):
    ...
    X, y = expand_data_geneator_by_sampling_rect(X, y, config)
    while True:
        array_indices = np.arange(len(X))
        for batch_index in range(config.batch_per_epoch):
...
def expand_data_geneator_by_sampling_rect(X,y,config):
    new_data = []
    new_label = []
...
def exapand_data_by_sampling_rect(data,config):
    new_data = []
    ...
    for segment,index in zip(data[0],range(len(data[0]))):
        ...
        for key in new_labels_dict.keys():
            new_label_list = [data[1][key][index]] * len(expanded_iq)
...
def exapand_test_by_sampling_rect(data,config):
    new_data = []
    new_labels_dict = collections.OrderedDict()
...
def load_data(config):
    ...
    if config.with_rect_augmentation is True:
        # train_data = exapand_data_by_sampling_rect(data=train_data,config=config)
        validation_data = exapand_data_by_sampling_rect(data=validation_data, config=config)
