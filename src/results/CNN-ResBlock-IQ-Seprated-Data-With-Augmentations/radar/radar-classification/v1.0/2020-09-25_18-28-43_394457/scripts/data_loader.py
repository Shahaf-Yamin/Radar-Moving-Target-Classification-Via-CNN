...
def expand_data_by_all_aux_exp_set(data_parser, train_data):
    train_data_aux_exp = data_parser.load_all_experiment()
    X = np.concatenate((train_data[0], train_data_aux_exp[0]), axis=0)  # Stack the experiment data
    train_labels = train_data[1]
    train_aux_exp_labels = train_data_aux_exp[1]
    for key in train_labels.keys():
...
def expand_data_by_synthetic_set(data_parser, train_data):
    ...
    idx_animal = np.where(aux_syn_target == 0)[0].tolist()
...
def load_data(config):
    ...
    assert not (config.load_all_experiment and config.stable_mode)
    ...
    if config.load_all_experiment is True:
        train_data = expand_data_by_all_aux_exp_set(data_parser=data_parser, train_data=train_data)
