...
class DataSetParser(object):
    ...
    def PCA_expansion(self, iq_mat):
        ...
        for expansion_index in range(5):
            ...
            KL_mat[:, -NUMBER_OF_KL_COEEFIECENT:] = KL_mat[:, -NUMBER_OF_KL_COEEFIECENT:] + np.multiply(np.random.uniform(-0.25,0.25,(32, NUMBER_OF_KL_COEEFIECENT)), np.real(KL_mat[:, -NUMBER_OF_KL_COEEFIECENT:])) + \
                    1j * np.multiply(np.random.uniform(-0.25,0.25,(32, NUMBER_OF_KL_COEEFIECENT)), np.imag(KL_mat[:, -NUMBER_OF_KL_COEEFIECENT:]))
