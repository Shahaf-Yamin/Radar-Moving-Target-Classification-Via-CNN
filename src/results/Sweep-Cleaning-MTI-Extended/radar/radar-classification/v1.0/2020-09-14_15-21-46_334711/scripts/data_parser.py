...
class DataSetParser(object):
    ...
    def PCA_expansion(self, iq_mat):
        ...
        for expansion_index in range(5):
            ...
            KL_mat[:, :NUMBER_OF_KL_COEEFIECENT] = (np.real(KL_mat[:,:NUMBER_OF_KL_COEEFIECENT]) + np.sqrt(0.2) * np.multiply(np.real(KL_mat[:,:NUMBER_OF_KL_COEEFIECENT]),((1/(np.sqrt(2)))*np.random.randn(32, NUMBER_OF_KL_COEEFIECENT)))) + \
                                                   (1.0j * ( np.imag(KL_mat[:,:NUMBER_OF_KL_COEEFIECENT]) + np.sqrt(0.2) * np.multiply(np.imag(KL_mat[:,:NUMBER_OF_KL_COEEFIECENT]),((1/(np.sqrt(2)))*np.random.randn(32, NUMBER_OF_KL_COEEFIECENT)))))
