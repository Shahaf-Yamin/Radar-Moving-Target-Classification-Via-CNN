...
def print_sweep_by_parameter(hist_dict,param_name,metric_list, graph_path, title, ylabel=None):
  ...
  for metric in metric_list:
    ...
    for param_val in hist_dict[param_name].keys():
      metric_dict[metric][param_val] = []
      if 'val_auc' == metric:
        val_auc = [(key, value) for key, value in hist_dict[param_name][param_val].items() if 'val_auc' in key]
        metric_dict['val_auc'][param_val] = val_auc[0][1]
      elif 'auc' == metric:
        train_auc = [(key, value) for key, value in hist_dict[param_name][param_val].items() if
                     'auc' in key and 'val' not in key]
        metric_dict['auc'][param_val] = train_auc[0][1]
      else:
        metric_dict[metric][param_val] = hist_dict[param_name][param_val][metric]
  plt.figure()
  for metric in metric_dict.keys():
    for param_val in metric_dict[metric].keys():
      plt.plot(range(len(metric_dict[metric][param_val])), metric_dict[metric][param_val], linewidth=2,
            label='{}, {} = {}'.format(metric, param_name, param_val).format())
