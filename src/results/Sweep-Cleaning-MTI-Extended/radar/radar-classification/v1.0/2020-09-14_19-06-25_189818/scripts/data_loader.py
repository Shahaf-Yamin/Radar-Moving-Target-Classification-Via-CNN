...
def expand_data_by_pca_augmentation(data_parser,train_data, validation_segments_id_list):

    synthetic_data = data_parser.get_pca_synthetic_data(validation_segments_id_list=validation_segments_id_list)

    # Validate that the interpolated data set has no bias with the validation set
    for segment_id in synthetic_data[1]['segment_id']:
    ...
    X = np.concatenate((train_data[0], synthetic_data[0]), axis=0)  # Stack the interpolated data
    train_labels = train_data[1]
    train_synthetic_labels = synthetic_data[1]
    for key in train_labels.keys():
        train_labels[key].extend(train_synthetic_labels[key])
    labels = train_labels
...
def expand_data_by_interpolation_fast_axis(data_parser,train_data, validation_segments_id_list):
    ...
    interpolated_data = data_parser.get_interpolation_data_fast_axis(validation_segments_id_list=validation_segments_id_list)
...
def expand_data_by_interpolation_slow_axis(data_parser,train_data, validation_segments_id_list):
    ...
    interpolated_data = data_parser.get_interpolation_data_slow_axis(validation_segments_id_list=validation_segments_id_list)
...
def load_data(config):
    ...
    train_data = data_parser.get_dataset_by_snr('train','HighSNR')
    ...
    if config.with_pca_augmentation is True:
        train_data = expand_data_by_pca_augmentation(data_parser=data_parser, train_data=train_data,
