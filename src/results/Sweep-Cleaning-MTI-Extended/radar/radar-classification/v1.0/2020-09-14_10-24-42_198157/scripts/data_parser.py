...
class DataSetParser(object):
    ...
    def PCA_expansion(self, iq_mat):
        ...
        for expansion_index in range(5):
            ...
            KL_mat[:, -NUMBER_OF_KL_COEEFIECENT:] = KL_mat[:, -NUMBER_OF_KL_COEEFIECENT:] + np.random.randn(32, NUMBER_OF_KL_COEEFIECENT) * (np.max(np.real(KL_mat[:, -NUMBER_OF_KL_COEEFIECENT:]), axis=0)) / 3.0 + \
                    1j * np.random.randn(32, NUMBER_OF_KL_COEEFIECENT) * np.max(np.imag(KL_mat[:, -NUMBER_OF_KL_COEEFIECENT:]), axis=0) / 3.0
