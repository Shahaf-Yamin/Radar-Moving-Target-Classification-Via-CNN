...
class DataSetParser(object):
    ...
    def PCA_expansion(self, iq_mat):
        ...
        for expansion_index in range(5):
            ...
            KL_mat[:, -NUMBER_OF_KL_COEEFIECENT:] = np.real(KL_mat[:, -NUMBER_OF_KL_COEEFIECENT:]) * (1.0 + ((1/(np.sqrt(2) * VARIANCE_SCALING)))*np.random.randn(32, NUMBER_OF_KL_COEEFIECENT)) #+ 1j * np.multiply(np.imag(KL_mat[:, -NUMBER_OF_KL_COEEFIECENT:]), np.random.randn(32, NUMBER_OF_KL_COEEFIECENT))
            + np.imag(KL_mat[:, -NUMBER_OF_KL_COEEFIECENT:]) * 1.0j * (1.0 + (1.0/(np.sqrt(2) * VARIANCE_SCALING))*np.random.randn(32, NUMBER_OF_KL_COEEFIECENT)) #+ 1j * np.multiply(np.imag(KL_mat[:, -NUMBER_OF_KL_COEEFIECENT:]), np.random.randn(32, NUMBER_OF_KL_COEEFIECENT))
