...
class DataSetParser(object):
    ...
    def plot_histogram(self):
        ...
        ax1.plot(np.linspace(np.min(self.aux_background_data[0][0]), np.max(self.aux_background_data[0][0]), 32 * 126),
                 histogram)
        ax1.title('Freq Response Histogram', fontsize=30)
        ax1.legend('Background',loc="best")
        ax1.xlabel('Value', fontsize=20)
        ax1.ylabel('Count', fontsize=30)

        ax1.imshow(self.aux_background_data[0][0])
        ax1.title('Freq Response Spectrogram', fontsize=30)
        ax1.legend('Background',loc="best")
        ax1.xlabel('Slow Axis', fontsize=20)
        ax1.ylabel('Fast Axis', fontsize=30)
        ...
        plt.figure()
        plt.plot(np.linspace(np.min(self.train_data[0][0]), np.max(self.train_data[0][0]), 32 * 126),
                 histogram)
        plt.title('Freq Response Histogram', fontsize=30)
        plt.legend('Human', loc="best")
        plt.xlabel('Value', fontsize=20)
        plt.ylabel('Count', fontsize=30)
        plt.savefig('Human Histogram')

        histogram, bin_edges = np.histogram(self.train_data[0][0], bins=32 * 126,
                                            range=(np.min(self.train_data[0][0]), np.max(self.train_data[0][0])))
        plt.figure()
        plt.plot(np.linspace(np.min(self.train_data[0][-1]), np.max(self.train_data[0][0]), 32 * 126),
                 histogram)
        plt.title('Freq Response Histogram', fontsize=30)
        plt.legend('Animal', loc="best")
        plt.xlabel('Value', fontsize=20)
        plt.ylabel('Count', fontsize=30)
        plt.savefig('Animal Histogram')
