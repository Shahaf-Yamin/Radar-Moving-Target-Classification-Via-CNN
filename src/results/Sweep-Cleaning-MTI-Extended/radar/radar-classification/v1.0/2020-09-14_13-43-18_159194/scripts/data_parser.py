...
class DataSetParser(object):
    ...
    def PCA_expansion(self, iq_mat):
        ...
        for expansion_index in range(5):
            ...
            KL_mat[:, -NUMBER_OF_KL_COEEFIECENT-10:-10] = np.real(KL_mat[:, -NUMBER_OF_KL_COEEFIECENT-SPACE_OF_CRUCIAL_COMPONENT:-SPACE_OF_CRUCIAL_COMPONENT]) * (1 + (1/np.sqrt(2))*np.random.randn(32, NUMBER_OF_KL_COEEFIECENT)) #+ 1j * np.multiply(np.imag(KL_mat[:, -NUMBER_OF_KL_COEEFIECENT:]), np.random.randn(32, NUMBER_OF_KL_COEEFIECENT))
