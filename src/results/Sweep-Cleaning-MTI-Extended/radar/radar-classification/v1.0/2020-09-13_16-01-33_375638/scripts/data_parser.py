...
class DataSetParser(object):
    ...
    def plot_histogram(self):
        ...
        plt.figure()
        plt.plot(np.linspace(np.min(self.aux_background_data[0][0]), np.max(self.aux_background_data[0][0]), 32 * 126),
                 histogram)
        plt.title('Freq Response Histogram', fontsize=30)
        plt.legend('Background',loc="best")
        plt.xlabel('Value', fontsize=20)
        plt.ylabel('Count', fontsize=30)
        plt.savefig('Background Histogram')
