...
class DataSetParser(object):
    ...
    def PCA_expansion(self, iq_mat):
        ...
        '''
        NUMBER_OF_KL_COEEFIECENT = 50
        new_iq_matrices = []
        for expansion_index in range(5):
            KL_mat = np.matmul(np.transpose(np.conjugate(standardized_iq_mat)), eigen_vector)
            KL_mat[:, -NUMBER_OF_KL_COEEFIECENT:] = KL_mat[:, -NUMBER_OF_KL_COEEFIECENT:] + np.random.randn(32, NUMBER_OF_KL_COEEFIECENT) * (np.max(np.real(KL_mat[:, -NUMBER_OF_KL_COEEFIECENT:]), axis=0)) / 10.0 + \
                    1j * np.random.randn(32, NUMBER_OF_KL_COEEFIECENT) * np.max(np.imag(KL_mat[:, -NUMBER_OF_KL_COEEFIECENT:]), axis=0) / 10.0
