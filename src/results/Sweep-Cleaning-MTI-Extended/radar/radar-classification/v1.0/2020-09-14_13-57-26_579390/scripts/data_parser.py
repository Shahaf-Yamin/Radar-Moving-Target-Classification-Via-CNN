...
class DataSetParser(object):
    ...
    def PCA_expansion(self, iq_mat):
        ...
        '''
        standardized_iq_mat = (iq_mat- np.mean(iq_mat))#/np.std(iq_mat)
        ...
        '''
        NUMBER_OF_KL_COEEFIECENT = 10
        SPACE_OF_LARGEST_COEFF = 5
        VARIANCE_SCALING = 100

        new_iq_matrices = []
        for expansion_index in range(5):
            KL_mat = np.matmul(np.transpose(np.conjugate(standardized_iq_mat)), eigen_vector)
            # KL_mat[:, -NUMBER_OF_KL_COEEFIECENT:] = KL_mat[:, -NUMBER_OF_KL_COEEFIECENT:] + np.multiply(np.random.uniform(-VARIANCE_OF_NOISE,VARIANCE_OF_NOISE,(32, NUMBER_OF_KL_COEEFIECENT)), np.real(KL_mat[:, -NUMBER_OF_KL_COEEFIECENT:])) + \
            #       1j * np.multiply(np.random.uniform(-VARIANCE_OF_NOISE,VARIANCE_OF_NOISE,(32, NUMBER_OF_KL_COEEFIECENT)), np.imag(KL_mat[:, -NUMBER_OF_KL_COEEFIECENT:]))
            KL_mat[:, -NUMBER_OF_KL_COEEFIECENT-SPACE_OF_LARGEST_COEFF:-SPACE_OF_LARGEST_COEFF] = np.real(KL_mat[:, -NUMBER_OF_KL_COEEFIECENT-SPACE_OF_LARGEST_COEFF:-SPACE_OF_LARGEST_COEFF]) * (1.0 + ((1/(np.sqrt(2) * VARIANCE_SCALING)))*np.random.randn(32, NUMBER_OF_KL_COEEFIECENT)) #+ 1j * np.multiply(np.imag(KL_mat[:, -NUMBER_OF_KL_COEEFIECENT:]), np.random.randn(32, NUMBER_OF_KL_COEEFIECENT))
            + np.imag(KL_mat[:, -NUMBER_OF_KL_COEEFIECENT-SPACE_OF_LARGEST_COEFF:-SPACE_OF_LARGEST_COEFF]) * 1.0j * (1.0 + (1.0/(np.sqrt(2) * VARIANCE_SCALING))*np.random.randn(32, NUMBER_OF_KL_COEEFIECENT)) #+ 1j * np.multiply(np.imag(KL_mat[:, -NUMBER_OF_KL_COEEFIECENT:]), np.random.randn(32, NUMBER_OF_KL_COEEFIECENT))
