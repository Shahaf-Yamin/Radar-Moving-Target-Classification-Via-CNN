...
def build_trainer(model, data, config, exp_name_time):
    if config.trainer_name == "classification":
        # trainer = ClassificationTrainer(model, data, config)
        trainer = ClassificationTrainerKeras(model, data, config, exp_name_time)
    else:
...
class ClassificationTrainerKeras(object):
    def __init__(self, model, data, config, exp_name_time):
        self.model_train = model['train']
    ...
    def train(self):
        ...
        if self.config.callback != "None":
            ...
            if self.config.callback == "lr_scheduler":
                ...
                callback_list.append(ReduceLROnPlateau(monitor='val_auc', factor=0.7, verbose=1,
                                                       min_lr=0.05 * self.config.learning_rate,
                                                       patience=self.config.lr_patience))
            # model checkpoint to save best model
            checkpoint_dir = './Checkpoints/checkpoint_{}'.format(self.exp_name_time)
            checkpoint_filepath = '{}/model_checkpoint'.format(checkpoint_dir)
            if self.config.model_checkpoint:
                ...
                save_model_callback = ModelCheckpoint(filepath=checkpoint_filepath, save_weights_only=False,
            ...
            # else:
            steps_per_epoch = self.config.steps_per_epoch if self.config.load_pkl_data is False or self.config.steps_per_epoch_overwrite is True else None
