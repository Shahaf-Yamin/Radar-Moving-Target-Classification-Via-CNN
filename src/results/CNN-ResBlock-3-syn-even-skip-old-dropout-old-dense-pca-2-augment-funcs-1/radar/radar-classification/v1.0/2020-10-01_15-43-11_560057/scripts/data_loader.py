...
def expand_data_by_human_low_exp_set(data_parser, train_data, with_pca,with_time_shift):
...
def expand_data_by_synthetic_set(data_parser, train_data, with_pca,with_time_shift):
...
def load_data(config):
    ...
    if config.load_low_human_experiment is True:
        train_data = expand_data_by_human_low_exp_set(data_parser=data_parser, train_data=train_data,
                                                      with_pca=config.with_pca_augmentation,with_time_shift=config.with_phase_time_shift)
    ...
    if config.save_pkl_data is True:
        ...
        with open('{}_data_description.txt'.format(config.pickle_file_name),mode='w') as data_file:
