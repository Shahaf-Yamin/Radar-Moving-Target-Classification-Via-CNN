...
def expand_data_by_random_phase_shift(data_parser, train_data, validation_segments_id_list):
    # Validate that the interpolated data set has no bias with the validation set
...
def load_data(config):
    ...
    if config.with_phase_distortion is True:
        train_data = expand_data_by_random_phase_shift(data_parser=data_parser, train_data=train_data,
                                                          validation_segments_id_list=validation_data[1]['segment_id'])
