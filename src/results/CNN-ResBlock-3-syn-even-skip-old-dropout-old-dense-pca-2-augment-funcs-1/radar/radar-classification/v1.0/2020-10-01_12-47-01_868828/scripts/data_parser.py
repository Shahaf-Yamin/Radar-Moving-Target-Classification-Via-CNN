...
class DataSetParser(object):
    ...
    def dump_time_phase_distortion_data(self,pickle_name,csv_name,required_segments):
        os.chdir(self.dataset_dir)
        with open('{}'.format(pickle_name), 'rb') as pickle_file:
            raw_data = pickle.load(pickle_file)
            iq = self.generate_time_phase_distortion_data(pkl_raw_data=raw_data, required_segments=required_segments)

        with open('{}'.format(csv_name)) as csv_file:
            labels = csv.reader(csv_file, delimiter=',', quotechar='|')
            labels_dict = self.generate_time_phase_distortion_csv_data(csv_labels=labels, required_segments=required_segments)

        labels_dict['target_type'] = [0 if y == 'animal' else 1 for y in labels_dict['target_type']]

        return iq, labels_dict

    def generate_time_phase_distortion_data(self,pkl_raw_data,required_segments):
        doppler_burst = pkl_raw_data['doppler_burst']
        raw_iq_matrices = pkl_raw_data['iq_sweep_burst']
        raw_iq_segments = pkl_raw_data['segment_id'].tolist()
    ...
    def generate_time_phase_distortion_csv_data(self, csv_labels, required_segments):
        is_first_line = True
        labels_augmented = collections.OrderedDict()
