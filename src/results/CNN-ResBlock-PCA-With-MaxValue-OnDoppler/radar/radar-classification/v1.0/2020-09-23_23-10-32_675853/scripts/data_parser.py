...
class DataSetParser(object):
    ...
    def PCA_time_augmentation(self, iq):
        ...
        for x,index in zip(X_augment,range(len(X_augment))):
            ...
            plt.savefig('Augmentated IQ{}_normalized_error{}'.format(index,np.linalg.norm(np.cov(standardized_iq_mat) - np.cov(X_augment[4])) /np.linalg.norm(np.cov(standardized_iq_mat)) ))
