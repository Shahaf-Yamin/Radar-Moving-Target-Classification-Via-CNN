    ...
    def generate_pca_expansion_for_iq_samples(self,pkl_raw_data, validation_indices):
        ...
        if self.config.use_mti_improvement is True:
            ...
            for segment, segment_id in zip(range(0, raw_iq_matrices.shape[0]), train_segments_id):
                for pca_index, current_iq_mat in zip(range(expansion_size), self.PCA_time_augmentation(raw_iq_matrices[segment_id])):
                    if 6660 == (segment + pca_index):
                        print('Failure')
                    iq[segment + pca_index] = self.fft(current_iq_mat)
                    if self.config.with_max_value_on_doppler is True:
                        iq[segment + pca_index] = self.max_value_on_doppler(iq=iq[self.config.num_of_pca_segments * segment + pca_index], doppler_burst=doppler_burst[segment])
                    else:
                        iq[segment + pca_index] = np.fft.fftshift(iq[segment + pca_index])
