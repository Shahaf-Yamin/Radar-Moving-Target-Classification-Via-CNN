...
def kalman_filter_clean_algorithm(iq_mat):
    ...
    secound_diag = np.ones((1,iq_mat.shape[0]-1))[0]
    third_diag = np.ones((1,iq_mat.shape[0]-2))[0]
    forth_diag = np.ones((1,iq_mat.shape[0]-3))[0]
    Filter.H = np.diag(main_diag,0) + 0.8 * np.diag(secound_diag,1) + 0.8 * np.diag(secound_diag,-1) + 0.7 * np.diag(third_diag,2) \
               + 0.7 * np.diag(third_diag, -2) + 0.6 * np.diag(forth_diag, 3) + 0.6 * np.diag(forth_diag, -3)
    ...
    for slow_axis_index in range(0, iq_mat.shape[1]):
        iq_mat[:, slow_axis_index] = iq_mat[:, slow_axis_index] - 0.3 * Filter.x
