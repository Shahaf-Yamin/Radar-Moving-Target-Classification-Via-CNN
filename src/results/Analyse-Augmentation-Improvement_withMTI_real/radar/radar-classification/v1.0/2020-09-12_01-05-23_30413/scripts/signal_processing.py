...
def kalman_filter_clean_algorithm(iq_mat):
    ...
    Filter.H = np.diag(main_diag,0) + np.diag(secound_diag,1) + np.diag(secound_diag,-1) + np.diag(third_diag,2) \
               + np.diag(third_diag, -2) + 0.6 * np.diag(forth_diag, 3) + 0.6 * np.diag(forth_diag, -3)
