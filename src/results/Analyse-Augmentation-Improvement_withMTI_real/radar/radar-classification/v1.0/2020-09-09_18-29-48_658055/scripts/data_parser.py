...
class DataSetParser(object):
    ...
    def butter_lowpass_filter(self, data, cutoff, nyq, order):
        ...
        b, a = butter(order, normal_cutoff, btype='low', analog=False, fs=nyq * 2)
        y = filtfilt(b, a, data)
        return y

    def filter_noise(self,iq):
        # Filter requirements.
        T = 32.0  # Sample Period
        fs = 126.0  # sample rate, Hz
        cutoff = 35  # desired cutoff frequency of the filter,

        nyq = 0.5 * fs  # Nyquist Frequency
        for timestamp in range(iq.shape[1]):
            iq[:,timestamp] = self.butter_lowpass_filter(data=iq[:,timestamp], cutoff=cutoff, nyq=nyq, order=4)
        return iq
    ...
    def preprocess_pkl_data(self, pkl_raw_data):
        ...
        if self.config.use_mti_improvement is True:
            ...
            for segment in range(raw_iq_matrices.shape[0]):
                ...
                iq[segment] = self.filter_noise(iq=iq[segment])
    ...
    def interpolation_fast_axis(self, iq_time_matrix):
        ...
        '''
        interpolated_iq_time_func_real = [interpolate.interp1d(fast_axis,np.real(iq_normalized[:,timestep_index]),kind='cubic') for timestep_index in range(iq_normalized.shape[1])]
        interpolated_iq_time_func_imag = [interpolate.interp1d(fast_axis,np.imag(iq_normalized[:,timestep_index]),kind='cubic') for timestep_index in range(iq_normalized.shape[1])]
    ...
    def interpolation_slow_axis(self, iq_time_matrix):
        ...
        Generating the interpolator object
        '''
        interpolated_iq_time_func_real = [interpolate.interp1d(slow_axis,np.real(iq_normalized[freq_index,:]),kind='cubic') for freq_index in range(iq_normalized.shape[0])]
        interpolated_iq_time_func_imag = [interpolate.interp1d(slow_axis,np.imag(iq_normalized[freq_index,:]),kind='cubic') for freq_index in range(iq_normalized.shape[0])]
