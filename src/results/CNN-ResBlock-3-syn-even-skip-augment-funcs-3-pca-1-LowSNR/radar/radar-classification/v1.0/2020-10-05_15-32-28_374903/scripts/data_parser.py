...
class DataSetParser(object):
    ...
    def fft(self, iq, window_type='hann', axis=0, window=None):
        ...
        window_option = {'hann': self.hann, 'blackman': self.blackman, 'blackman_nutall': self.blackman_nutall, 'rectangle': self.rect,
                         'blackman_harris': self.blackman_harris}

        assert window_type == 'hann' or window_type == 'blackman' or window_type == 'blackman_nutall' or window_type == 'rectangle' or window_type == 'blackman_harris'
    ...
    def dump_time_shift_using_phase_data(self, pickle_name, csv_name, validation_indices):
        ...
        with open('{}'.format(csv_name)) as csv_file:
            labels = csv.reader(csv_file, delimiter=',', quotechar='|')
            labels_dict = self.generate_time_shift_using_phase_csv_data(csv_labels=labels,
    ...
    def generate_window_data(self, pkl_raw_data, validation_indices):
        ...
        for index, segment_id in enumerate(train_segments_id):
            ...
            org_iq = self.fft(raw_iq_matrices[idx], window_type='blackman_nutall')
            print('error for window {} is {}'.format(self.config.default_window_type, np.linalg.norm(np.cov(self.normalize(iq[index])) - np.cov(self.normalize(org_iq))) / np.linalg.norm(
