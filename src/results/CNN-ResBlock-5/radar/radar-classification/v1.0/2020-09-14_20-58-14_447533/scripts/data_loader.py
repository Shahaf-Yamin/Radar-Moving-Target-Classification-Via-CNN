...
def augment(data, config):
    ...
    for augment_index in range(config.augment_expansion_number):
        if 'flip_image' in augment_funcs and augment_index == 0:
            data = np.concatenate(
                (data, np.array([augment_vertical_flip(sample[0], sample[1]) for sample in orig_data])), axis=0)
        if 'horiz_flip' in augment_funcs and augment_index == 0:
            data = np.concatenate((data, np.array([augment_horiz_flip(sample[0], sample[1]) for sample in orig_data])),
                                  axis=0)
        if 'normal' in augment_funcs:
            data = np.concatenate((data, np.array([augment_normal(sample[0], sample[1]) for sample in orig_data])),
                                  axis=0)
        if 'guassian_filt' in augment_funcs:
            data = np.concatenate(
                (data, np.array([augment_gaussian_filt_2d(sample[0], sample[1]) for sample in orig_data])), axis=0)
        if 'freq_shift' in augment_funcs:
            data = np.concatenate((data, np.array([augment_freq_shift(sample[0], sample[1]) for sample in orig_data])),
                                  axis=0)
        if 'timestep_shift' in augment_funcs:
            data = np.concatenate(
                (data, np.array([augment_timestep_shift(sample[0], sample[1]) for sample in orig_data])), axis=0)
        if 'pca' in augment_funcs:
            data = np.concatenate(
                (data, np.array([augment_with_pca(sample[0], sample[1]) for sample in orig_data])), axis=0)
    return data
