...
class DataSetParser(object):
    ...
    def split_public_test_valid(self):
        ...
        for key in self.validation_data[1].keys():
            y_test[key] = self.validation_data[1][key][idx_test]
        ...
        for key in self.validation_data[1].keys():
            y_train[key] = self.validation_data[1][key][idx_train]

        train_data = (X_train, y_train)

        #y_train = [1 if y == 'human' else 0 for y in np.array(self.validation_data[1]['target_type'])[idx_train].tolist()]
        # train_data_public = np.array([(iq_mat, y) for iq_mat, y in zip(X_train, y_train)])
