        ...
        if self.config.with_interpolation is True:
            ...
            for line in csv_labels:
                if is_first_line:
                    ...
                    line_index = 0
                    for key, data, line_index in zip(labels_augmented.keys(), line):
                        for index in range(self.config.interpolation_size):
                            # Duplicate the data
                            if key == 'segment_id':
                                labels_augmented[key].append(str(-int(csv_labels[key][line_index])))
                            else:
                                labels_augmented[key].append(csv_labels[key][line_index])
