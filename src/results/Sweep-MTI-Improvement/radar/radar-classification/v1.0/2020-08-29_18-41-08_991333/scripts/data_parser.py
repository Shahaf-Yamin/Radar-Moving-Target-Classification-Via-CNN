...
class DataSetParser(object):
    ...
    def interpolation(self, iq_time_matrix):
        ...
        for index in range(self.config.interpolation_size):
            start_index = index
            fast_stop_index = self.config.interpolation_size * 127
            current_fast_axis = np.arange(start_index, fast_stop_index, self.config.interpolation_size)
            interpolated_iq_mat_real = np.stack([item(np.take(fast_axis_interpolated,current_fast_axis)) for item in interpolated_iq_time_func_real])
            interpolated_iq_mat_imag = np.stack([item(np.take(fast_axis_interpolated,current_fast_axis)) for item in interpolated_iq_time_func_imag])
