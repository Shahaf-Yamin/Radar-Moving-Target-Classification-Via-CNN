...
class DataSetParser(object):
    ...
    def generate_pca_expansion_for_iq_samples(self,pkl_raw_data, validation_indices):
        ...
        if self.config.with_iq_matrices is True or self.config.with_magnitude_phase is True:
            iq = np.zeros((expansion_size * raw_iq_matrices.shape[0] - len(validation_indices), raw_iq_matrices.shape[1] - 2, raw_iq_matrices.shape[2], 2))
    ...
    def generate_pca_synthetic_expansion_for_iq_samples(self, pkl_raw_data, required_indices):
        ...
        if self.config.with_iq_matrices is True or self.config.with_magnitude_phase is True:
            iq = np.zeros((expansion_size * len(required_indices), raw_iq_matrices.shape[1] - 3, raw_iq_matrices.shape[2],2))
            ...
            iq = np.zeros((expansion_size * len(required_indices), raw_iq_matrices.shape[1] - 2, raw_iq_matrices.shape[2]))
