...
class DataSetParser(object):
    ...
    def preprocess_pkl_data(self, pkl_raw_data):
        ...
        if self.config.use_mti_improvement is True:
            if self.config.with_iq_matrices is True or self.config.with_magnitude_phase is True:
                iq = np.zeros((raw_iq_matrices.shape[0], raw_iq_matrices.shape[1] - 3, raw_iq_matrices.shape[2],2))
            else:
    ...
    def generate_pca_expansion_for_iq_samples(self,pkl_raw_data, validation_indices):
        ...
        if self.config.use_mti_improvement is True:
            ...
            With MTI
            '''
            iq = np.zeros((expansion_size * raw_iq_matrices.shape[0] - len(validation_indices), raw_iq_matrices.shape[1] - 3, raw_iq_matrices.shape[2]))
            for segment, segment_id in zip(range(0, iq.shape[0], expansion_size), train_segments_id):
                for pca_index, current_iq_mat in zip(range(expansion_size), self.PCA_time_augmentation(raw_iq_matrices[segment_id])):
                    iq[self.config.num_of_pca_segments * segment + pca_index] = self.fft(self.mti_improvement(current_iq_mat))
                    if self.config.with_max_value_on_doppler is True:
                        iq[self.config.num_of_pca_segments * segment + pca_index] = self.max_value_on_doppler(iq=iq[self.config.num_of_pca_segments * segment + pca_index], doppler_burst=doppler_burst[segment])
                    else:
                        iq[self.config.num_of_pca_segments * segment + pca_index] = np.fft.fftshift(iq[self.config.num_of_pca_segments * segment + pca_index])
                    iq[self.config.num_of_pca_segments * segment + pca_index] = self.normalize(iq=iq[self.config.num_of_pca_segments * segment + pca_index])

        else:
            '''
            Without MTI
            '''
            iq = np.zeros((expansion_size * raw_iq_matrices.shape[0] - len(validation_indices), raw_iq_matrices.shape[1] - 2, raw_iq_matrices.shape[2]))
            for segment, segment_id in zip(range(0, raw_iq_matrices.shape[0]), train_segments_id):
                for pca_index, current_iq_mat in zip(range(expansion_size), self.PCA_time_augmentation(raw_iq_matrices[segment_id])):
                    iq[self.config.num_of_pca_segments * segment + pca_index] = self.fft(current_iq_mat)
                    if self.config.with_max_value_on_doppler is True:
                        iq[self.config.num_of_pca_segments * segment + pca_index] = self.max_value_on_doppler(iq=iq[self.config.num_of_pca_segments * segment + pca_index], doppler_burst=doppler_burst[segment])
                    else:
                        iq[self.config.num_of_pca_segments * segment + pca_index] = np.fft.fftshift(iq[self.config.num_of_pca_segments * segment + pca_index])
                    iq[self.config.num_of_pca_segments * segment + pca_index] = self.normalize(iq=iq[pca_index + self.config.num_of_pca_segments * segment])
        return iq
    ...
    def generate_pca_synthetic_expansion_for_iq_samples(self, pkl_raw_data, required_indices):
        doppler_burst = pkl_raw_data['doppler_burst']
        raw_iq_matrices = pkl_raw_data['iq_sweep_burst']
        expansion_size = self.config.num_of_pca_segments
        train_segments_id = required_indices
        ...
        if self.config.use_mti_improvement is True:
            ...
            '''
            iq = np.zeros(
                (expansion_size * len(required_indices), raw_iq_matrices.shape[1] - 3, raw_iq_matrices.shape[2]))
            for segment, segment_id in zip(range(0, iq.shape[0], expansion_size), train_segments_id):
                for pca_index, current_iq_mat in zip(range(expansion_size), self.PCA_time_augmentation(raw_iq_matrices[segment_id])):
                    iq[self.config.num_of_pca_segments * segment + pca_index] = self.fft(current_iq_mat)
                    if self.config.with_max_value_on_doppler is True:
                        iq[self.config.num_of_pca_segments * segment + pca_index] = self.max_value_on_doppler(
                            iq=iq[self.config.num_of_pca_segments * segment + pca_index], doppler_burst=doppler_burst[segment])
                    else:
                        iq[self.config.num_of_pca_segments * segment + pca_index] = np.fft.fftshift(
                            iq[self.config.num_of_pca_segments * segment + pca_index])
                    iq[self.config.num_of_pca_segments * segment + pca_index] = self.normalize(
                        iq=iq[pca_index + self.config.num_of_pca_segments * segment])
            ...
            '''
            iq = np.zeros(
                (expansion_size * len(required_indices), raw_iq_matrices.shape[1] - 2, raw_iq_matrices.shape[2]))
            for segment, segment_id in zip(range(0, len(required_indices)), train_segments_id):
                for pca_index, current_iq_mat in zip(range(expansion_size), self.PCA_time_augmentation(raw_iq_matrices[segment_id])):
                    iq[self.config.num_of_pca_segments * segment + pca_index] = self.fft(current_iq_mat)
                    if self.config.with_max_value_on_doppler is True:
                        iq[self.config.num_of_pca_segments * segment + pca_index] = self.max_value_on_doppler(
    ...
    def generate_pca_synthetic_csv_data(self, csv_labels, required_indices):
        is_first_line = True
        labels_augmented = collections.OrderedDict()
    ...
    def dump_PCA_synthetic_data(self, pickle_name, csv_name, required_indices):
        os.chdir(self.dataset_dir)
        with open('{}'.format(pickle_name), 'rb') as pickle_file:
            raw_data = pickle.load(pickle_file)
            iq = self.generate_pca_synthetic_expansion_for_iq_samples(pkl_raw_data=raw_data, required_indices=required_indices)

        with open('{}'.format(csv_name)) as csv_file:
            labels = csv.reader(csv_file, delimiter=',', quotechar='|')
            labels_dict = self.generate_pca_synthetic_csv_data(csv_labels=labels, required_indices=required_indices)
