...
class DataSetParser(object):
    def __init__(self, stable_mode, config, read_test_only=False):
        ...
        if stable_mode or self.config.load_all_experiment:
        ...
        if self.config.load_synthetic:
            ...
            if self.config.with_pca_augmentation is True:
                synt_pca_expansion = self.dump_PCA_data(
                    pickle_name='MAFAT RADAR Challenge - Auxiliary Synthetic Set V2.pkl',
                    csv_name='MAFAT RADAR Challenge - Auxiliary Synthetic Set V2.csv', validation_indices=[])
                X = np.concatenate((self.aux_syn[0], synt_pca_expansion[0]), axis=0)
                for key in self.aux_syn[1].keys():
                    self.aux_syn[1][key].extend(synt_pca_expansion[1][key])
                self.aux_syn = (X, self.aux_syn[1])
