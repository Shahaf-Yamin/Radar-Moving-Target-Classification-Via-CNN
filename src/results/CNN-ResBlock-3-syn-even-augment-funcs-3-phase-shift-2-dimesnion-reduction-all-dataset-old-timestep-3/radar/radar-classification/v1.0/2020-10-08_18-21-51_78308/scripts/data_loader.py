...
def load_data(config):
    ...
    if config.load_pkl_data:
        ...
        if config.with_pca_dimension_reducation is True:
            train_data = PCA_dimension_reducation(train_data[0], data_parser)
        ...
        if config.save_pkl_data is True:
            ...
            pickle.dump([train_data[:,0], train_data[:,1]], open("Organized_Data_in_pickle/{}_train_{}_segments.pkl".format(config.pickle_file_name,len(train_data)), "wb"), protocol=pickle.HIGHEST_PROTOCOL)
            pickle.dump([validation_data[:,0], validation_data[:,1]], open("Organized_Data_in_pickle/{}_validation_{}_segments.pkl".format(config.pickle_file_name,len(validation_data)), "wb"), protocol=pickle.HIGHEST_PROTOCOL)
            ...
            print("dumped validation data to file: Organized_Data_in_pickle/{}_validation_{}_segments.pkl".format(config.pickle_file_name,len(validation_data)))
    ...
    NUMBER_OF_SEGEMENTS_AFTER_AUGMENTION = NUMBER_OF_SEGEMENTS_BEFORE_AUGMENTION * (len(config.augment_funcs)+1)
...
def read_data(config):
    ...
    if config.use_public_test_set:
        ...
        validation_data = data_parser.get_dataset_by_snr(dataset_type='test', snr_type=config.snr_type)
    else:
...
def transform_per_epoch(data, config):
    def augment_normal(iq_mat, label):
        iq_mat = iq_mat + tf.random.normal(shape=np.array(iq_mat.shape),mean=config.augment_normal_mean,stddev=config.augment_normal_std,dtype=tf.float32)
        return iq_mat, label

    def augment_timestep_shift(iq_mat, label):
        shift = tf.cast(tf.sign(tf.random.normal(shape=np.array([1]),mean=0,stddev=1,dtype=tf.float32)), dtype=tf.int32) * tf.constant(config.timestep_shift_delta,dtype=tf.int32)
        # shift = np.random.choice(
        #     [x for x in range(-config.timestep_shift_delta, config.timestep_shift_delta + 1) if x != 0])
        return tf.roll(iq_mat, shift[0], axis=1), label
    ...
    noise_data = data['train'].map(augment_normal)
    step_shift_data = data['train'].map(augment_timestep_shift)
...
def make_iterators(data, config):
    ...
    n_train = 150000
    if config.augment_per_epoch is True:
        train_iter = data['train'].batch(config.batch_size, drop_remainder=True).prefetch(n_train + 1000).take(-1)
    else:
        train_iter = data['train'].shuffle(n_train + 1000).batch(config.batch_size, drop_remainder=True).prefetch(n_train + 1000).take(-1)
