    ...
    def train(self):
        ...
        if self.config.callback != "None":
            ...
            history = self.model_train.fit(self.data['train'], epochs=self.config.num_epochs,
                                           ...
                                           verbose=self.config.fit_verbose, steps_per_epoch=len(
                    list(self.data['train'].as_numpy_iterator())) // self.config.num_epochs if self.config.augment_per_epoch else None)
            if self.config.model_checkpoint:
