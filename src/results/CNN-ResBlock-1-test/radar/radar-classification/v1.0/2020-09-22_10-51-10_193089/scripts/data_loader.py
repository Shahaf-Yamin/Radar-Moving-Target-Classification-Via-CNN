...
def generate_test_set(train_data, TRACK_ID_TEST_SET_SIZE):
    ...
    high_data, high_indices = get_data_by_snr(snr_type='HighSNR',X_sorted=X_sorted_snr,labels_sorted=labels_sorted_snr)
    low_data, low_indices = get_data_by_snr(snr_type='LowSNR', X_sorted=X_sorted_snr, labels_sorted=labels_sorted_snr)
    ...
    mix_idx = np.bool_(np.ones(len(X_sorted_snr)))
    mix_idx[high_indices.append(low_indices)] = False
    X_train_mixed = X_sorted_snr[mix_idx]
    labels_train_mixed = collections.OrderedDict()
    for key in train_data[1].keys():
        labels_train_mixed[key] = (np.array(labels_sorted_snr[key])[mix_idx]).tolist()

    # sort high_data and low_data by track_id
    X_sorted_high, labels_sorted_high = sort_data_by_label(high_data, label='track_id')
    X_sorted_low, labels_sorted_low = sort_data_by_label(low_data, label='track_id')
    ...
    imax_valid_animal_high = get_i_to_split(target_data=animal_data_high, count=TRACK_ID_TEST_SET_SIZE)
    imax_valid_human_high = get_i_to_split(target_data=human_data_high, count=TRACK_ID_TEST_SET_SIZE)
    ...
    # imax is the last index (INCLUDING!) of the validation data
    imax_valid_animal_low = get_i_to_split(target_data=animal_data_low, count=TRACK_ID_TEST_SET_SIZE)
    imax_valid_human_low = get_i_to_split(target_data=human_data_low, count=TRACK_ID_TEST_SET_SIZE)
    ...
    X_valid = np.concatenate((X_valid_human_high, X_valid_animal_high, X_valid_human_low, X_valid_animal_low), axis=0)
    X_train = np.concatenate((X_train_human_high, X_train_animal_high, X_train_human_low, X_train_animal_low,X_train_mixed), axis=0)
    ...
    for key in train_data[1].keys():
        ...
        labels_valid[key].extend(labels_valid_human_low[key])
        ...
        labels_train[key].extend(labels_train_human_low[key])
...
def expand_human_data_by_tracks(train_data, config):
    ...
    X_sorted, labels_sorted = sort_data_by_label(train_data, label='track_id')
